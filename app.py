...
def main() -> None:
    ...
    with st.spinner("Processing resumes..."):
        ...
        if clf is None or vectorizer is None:
            st.warning(
                ...
                "The app will still work using similarity and skills only."
            )
        ...
        for uploaded in uploaded_files:
            if not uploaded.name.lower().endswith(".pdf"):
                st.warning(f"Skipping non-PDF file: {uploaded.name}")
                continue

            text = extract_text_from_pdf(uploaded)
            if not text:
                st.warning(f"No extractable text found in {uploaded.name}.")
                continue

            resume_names.append(uploaded.name)
            resume_texts.append(text)
            ai_summary, ai_summary_error = summarize_resume_ai(text)
            summaries.append(ai_summary)
            summary_errors.append(ai_summary_error or "")

        if not resume_texts:
            st.error("No usable resumes after parsing. Please check your files.")
            return

        sim_scores_raw = compute_job_resume_similarity(job_description, resume_texts)
        tfidf_scores_pct = [s * 100 for s in sim_scores_raw]
        bert_scores_raw, bert_error = compute_bert_similarity(job_description, resume_texts)
        if bert_error:
            st.warning(
                "Deep Learning model could not be loaded. Falling back to TF-IDF only "
                f"for semantic scoring. Details: {bert_error}"
            )
            bert_scores_raw = sim_scores_raw.copy()
        bert_scores_pct = [s * 100 for s in bert_scores_raw]
        ...
        for i in range(len(resume_texts)):
            ...
            # Deep-learning combined score required by specification.
        ...
        duplicates = [
            (i, j, s) for (i, j, s) in duplicate_pairs if s >= 0.9
        ]
    ...
    for idx, name in enumerate(resume_names):
        skills_str = ", ".join(
            skill_sets_to_strings(skill_analyses[idx].detected_skills)
        )
        missing_str = ", ".join(
            skill_sets_to_strings(skill_analyses[idx].missing_skills)
        )
    ...
    for _, row in results_df.iterrows():
        with st.expander(f"{row['Candidate']} — {row['Combined Final Score %']:.2f}%"):
            ...
            if not gemini_api_key:
                st.info("Enter your Gemini API Key in the sidebar to unlock GenAI Deep Analysis, Interview Questions, and Email Drafts.")
            else:
                ...
                with tab1:
                    ...
                    if st.button("Generate Deep Analysis", key=f"btn_{analysis_key}"):
                        with st.spinner("Analyzing candidate..."):
                            st.session_state[analysis_key] = generate_candidate_analysis(row["Raw Text"], job_description)
                ...
                with tab2:
                    ...
                    if st.button("Generate Interview Questions", key=f"btn_{questions_key}"):
                        with st.spinner("Generating questions..."):
                            st.session_state[questions_key] = generate_interview_questions(row["Raw Text"], job_description)
                ...
                with tab3:
                    ...
                    with col1:
                        if st.button("Draft Acceptance Email", key=f"btn_{email_accept_key}"):
                            with st.spinner("Drafting email..."):
                                st.session_state[email_accept_key] = generate_email_draft(row["Candidate"], row["Raw Text"], job_description, "accept")
                    ...
                    with col2:
                        if st.button("Draft Rejection Email", key=f"btn_{email_reject_key}"):
                            with st.spinner("Drafting email..."):
                                st.session_state[email_reject_key] = generate_email_draft(row["Candidate"], row["Raw Text"], job_description, "reject")
    ...
    if duplicates:
        ...
        dup_df = pd.DataFrame(
            dup_rows, columns=["Resume A", "Resume B", "Similarity"]
        )
